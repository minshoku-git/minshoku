...
export const UserDetailComponent = (): JSX.Element => {
  ...
  const pullBackHandler = () => {
    ...
    setDialogActionHandler(() => pullbackMutateHandler);
    setOpenDialog(true);
  };

  const pullbackMutateHandler: SubmitHandler<UserDetailFormValues> = async (data) => {
          ...
          {dataLoaded && userData && (
            <form onSubmit={handleSubmit(updateHandler)}>
              <Grid container rowSpacing={2} columnSpacing={{ xs: 1, sm: 2, md: 3 }} direction="column">
                ...
                {userData.t_companies.optional_item_title_1 && userData.optional_item_answer_1 && (
                ...
                {userData.t_companies.optional_item_title_2 && userData.optional_item_answer_2 && (
                ...
                <ItemBase name={'登録ステータス'} isRequired={2}>
                  <TextField
                    ...
                    sx={{ backgroundColor: 'lightgray' }}
                    disabled
                    value={convertUserRegistrationStatusName(userData.user_registration_status as UserRegistrationStatus)}
                  />
                </ItemBase>
                {userData?.user_registration_status === UserRegistrationStatus.REGISTERED && (
                  <>
              ...
              <Grid size={{ xs: 12 }} sx={{ display: 'flex', mt: 2, gap: 2 }}>
                ...
                {/* 否認の場合 */}
                {userData.user_registration_status === UserRegistrationStatus.DISAPPROVAL && (
                  <>
                    <Button
                      fullWidth
                      variant="contained"
                ...
                {/* 承認待ちの場合 */}
                {userData.user_registration_status === UserRegistrationStatus.WAITING_APPROVAL && (
                  <>
                    <Button
                      fullWidth
                      variant="contained"
