...
export const ShopDetailSchemaType = z.object({
  ...
  //   }),
  /* ステータス */
  usage_state: z.string().nonempty({ message: formatString(MSG_REQUIRED, 'ステータス') }),
  /* メモ */
  memo: z
    .string()
...
export const CompanyDetailSchema = z
  .object({
    ...
    /* 注文期限(時) */
    order_period_hour: z.string().nonempty({ message: formatString(MSG_REQUIRED, '時') }),
    /* 注文期限(分) */
    order_period_minute: z.string().nonempty({ message: formatString(MSG_REQUIRED, '分') }),
    /* キャンセル期限(日付) */
    cancel_period_day: z.string().nonempty({ message: formatString(MSG_REQUIRED, '日') }),
    /* キャンセル期限(時間) */
    cancel_period_hour: z.string().nonempty({ message: formatString(MSG_REQUIRED, '時') }),
    /* キャンセル期限(分) */
    cancel_period_minute: z.string().nonempty({ message: formatString(MSG_REQUIRED, '分') }),
  })
  /* カスタムバリデーション
  ------------------------------------------------------------------ */
