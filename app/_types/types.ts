...
import { CompaniesUsageStatus, SortType } from "./enum";
...
export const ShopDetailSchemaType = z.object({
  ...
  shop_post_code: z
    ...
    .regex(
      new RegExp(REG_POSTALCODE),
      formatString(MSG_POSTALCODE, "郵便番号")
    ),
  /* 都道府県 */
  shop_prefectures: z
    .string()
    .nonempty({ message: formatString(MSG_REQUIRED, "都道府県") }),
  /* 市区 */
  shop_municipalities: z
    .string()
    .nonempty({ message: formatString(MSG_REQUIRED, "市区") }),
  /* 町村 */
  shop_town_area: z
    .string()
    .nonempty({ message: formatString(MSG_REQUIRED, "町村") }),
  /* 番地 */
  ...
  usage_state: z
    .string()
    .nonempty({ message: formatString(MSG_REQUIRED, "ステータス") }),
  /* メモ */
  memo: z
    .string()
...
export const CompanySearchSchema = z.object({
  ...
  town_area: z.string().optional(),
  /* 利用ステータス */
  companies_usage_state: z.nativeEnum(CompaniesUsageStatus).optional(),
});
...
export const CompanyDetailSchema = z
  .object({
    ...
    order_period_day: z
      .string()
      .nonempty({ message: formatString(MSG_REQUIRED, "日") }),
    /* 注文期限(時間) */
    order_period_time: z.union([
      z.null({ message: formatString(MSG_INVALID, "時間") }),
      z.date({
        errorMap: (issue, {}) => ({
          message:
            issue.code === "invalid_date"
              ? formatString(MSG_INVALID, "時間")
              : formatString(MSG_REQUIRED, "時間"),
        }),
      }),
    ]),
    /* キャンセル期限(日付) */
    ...
    /* 会社利用ステータス */
    companies_usage_state: z.nativeEnum(CompaniesUsageStatus),
  })
  /* カスタムバリデーション
  ------------------------------------------------------------------ */
  ...
  .refine(
    (data) =>
      !(data.optional_item_title_1 === "" && data.optional_item_notes_1 !== ""),
    {
  ...
  .refine(
    (data) =>
      !(data.optional_item_title_2 === "" && data.optional_item_notes_2 !== ""),
    {
  ...
  .superRefine((data, ctx) => {
    data.departmentInfo.forEach((item, index) => {
      const filterLength = data.departmentInfo.filter(
        (f) => f.name === item.name
      ).length;
      if (filterLength > 1) {
        ctx.addIssue({
  ...
  .superRefine((data, ctx) => {
    data.employmentStatusInfo.forEach((e, index) => {
      if (
        ...
        (e.deduction_flag || e.credit_flag || e.paypay_flag)
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: [`employmentStatusInfo.${index}.employment_status_name`],
      ...
      if (!regex1.test(e.set_meal_burden)) {
        ctx.addIssue({
          ...
          message: formatString(MSG_REQUIRED, "あうとー！"),
        });
      }
    });
  })
...
export const UserDetailSchema = z.object({
  ...
  restriction: z
    .string()
    .nonempty({ message: formatString(MSG_REQUIRED, "利用制限") }),
  /* メモ */
  memo: z
    .string()
    .max(500, formatString(MSG_MAX, "メモ", "500"))
